...
::GenerateData()
{
  ...
  //constrain the range of otsu
  otsu->SetLowerBound(m_LowerBound);
  otsu->SetUpperBound(m_UpperBound);
  otsu->SetMaskOnOff(isMask);
  otsu->SetMask(m_Mask);
  
  isMask ? otsu->ComputeSVRCWithMask() : otsu->ComputeSVRC();

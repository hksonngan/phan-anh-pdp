...
void loadGraphEdges(int iw, int ih){
  int n,m;
...
runTest( int argc, char** argv) 
{
    ...
    unsigned int num_threads = NUMTHREADS;//512;
...
//	printf("Before loading\n");   
    loadGraphEdges(iw,ih);
//	printf("After loading\n");
//    loadGraph(iw,ih);
//	printf("After loading\n");
    //allocate Bucket memory

//   tex = (    texture<float, 2, cudaReadModeElementType>*) malloc(4*sizeof(    texture<float, 2, cudaReadModeElementType>));
...
//    hBi[31]=8;
    ...
    int * dS;
    cudaMalloc( (void**) &dS, 16*BUCKETSIZE*sizeof(int));
    int * hS= (int*) malloc(16*BUCKETSIZE*sizeof(int));
    int * dSCount;
...
//      if(RCount[0]==0) i++;
        ...
        labelKernel    <<<grid, threads >>> ( i, dB, BCount,dBPos, iw,ih, dRLoc, dR, dDistR, dDist,dBucketMap);
...
//        printf("Sending RCount %d\n",4*4*hBPos[i]);
        ...
        relaxKernel    <<<grid, threads >>> ( 4*4*hBPos[i], dB, BCount, dBPos, dRLoc, dR, dDistR, dDist,dBucketPos, dBucketMap,lido);
        cudaThreadSynchronize();
...
/*	
 	...
 	static rmax = 0;
 	if(4*4*RCount[0]>rmax) rmax = 4*4*RCount[0];
